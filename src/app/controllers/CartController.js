...
class CartController {
    ...
    async removeFromCart(req, res) {
        ...
        try {
            const query = `
        DELETE FROM "GioHang" WHERE "KhachHangID" = $1 AND "SanPhamID" = $2;
        `
            await db.query(query, [KhachHangID, productIdInt]);
            res.send({ success: true, message: 'Sản phẩm đã được xóa khỏi giỏ hàng' });
        } catch (error) {
            console.error('Lỗi khi xóa sản phẩm khỏi giỏ hàng:', error);
            res.status(500).send({ success: false, message: 'Lỗi khi xóa sản phẩm khỏi giỏ hàng' });
        }
    }
}
